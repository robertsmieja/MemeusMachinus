...
export default class Blacklist {
    ...
    public static censorshipUserByIdOrTag = (user: string) => {
    ...
    public static censorshipInfoAddViolation = (user: User, word: string) => {
        ...
        Blacklist.censorshipToCSV();
    };

    public static censorshipInfoAddWarning = (user: User, word: string) => {
        if (!censorshipInfo[user.id]) {
            censorshipInfo[user.id] = {};
            censorshipInfo[user.id].tag = user.tag;
            censorshipInfo[user.id].violations = [];
    ...
    public static blacklistAlertMods = async (message: Message, violatingWord: string, warning: boolean) => {
        let violationChannel = message.guild.channels.find("name", "action-log") as TextChannel;
        let messageChannel = message.channel as TextChannel;
        let bars = "----------------------\n";
        if (!warning)
            return await violationChannel.send(bars + "Removed message with violation (`" + violatingWord + "`) in #" + messageChannel.name + " posted by `@" + message.author.tag + "`:\n`" + message.content + "`");
        else {
            return await violationChannel.send(bars + "Potential violation (`" + violatingWord + "`) in #" + messageChannel.name + " posted by `@" + message.author.tag + "`:\n`" + message.content + "`");
        }
    };

    public static handleBlacklist = async (message: Message, DiscordBotTag: string) => {
        ...
        if (censoredWord && message.author.tag !== DiscordBotTag && !misc.memberIsMod(message) && !(misc.memberIsMod(message) && message.content.startsWith("!blacklist")) && blacklistIgnore.indexOf(message.channel.id) == -1) {
            ...
            try {
                ...
                let warning = await message.channel.send("<@" + message.author.id + ">, please refrain from using that language on this server.") as Message;
                warning.delete(2250);
            }
            Blacklist.censorshipInfoAddViolation(message.author, censoredWord);
            await Blacklist.blacklistAlertMods(message, censoredWord, false);
            return true;
        }
        return false;
    };

    public static handleBlacklistPotential = async (message: Message, DiscordBotTag: string) => {
        let updatedText = message.content.toLowerCase().replace(/\n/g, " ");
        let strippedText = updatedText.replace(/[^a-z]/gi, '');
        ...
        if (potentialViolation && message.author.tag !== DiscordBotTag && !misc.memberIsMod(message) && !(misc.memberIsMod(message) && message.content.startsWith("!blacklist")) && blacklistIgnore.indexOf(message.channel.id) == -1) {
            await Blacklist.blacklistAlertMods(message, potentialViolation, true);
            Blacklist.censorshipInfoAddWarning(message.author, potentialViolation);
        }
    };

    //TODO add consistent return statements to get a consistent return type
    public static handleBlacklistCommands = async (message: Message, args: string[]): Promise<any> => {
        if (args[0] === "!blacklist" && misc.memberIsMod(message)) {
            if (args.length === 1) {
                ...
                if (args.length > 2) {
                    let user = Blacklist.censorshipUserByIdOrTag(args[2]);
                    if (user) {
